...
import { type Ingredient, type Cookware, type Step, type Metadata, type Item, type ShoppingList } from './cooklang';
        ...
        stepLoop: for (let i = 0; i < lines.length; i++) {
            ...
            for (let match of line.matchAll(tokens)) {
                ...
                if (groups.mIngredientName) {
                    const { quantity, wasFraction } = parseQuantity(groups.mIngredientQuantity);
                    const ingredient: Ingredient = {
                        type: 'ingredient',
                        name: groups.mIngredientName,
                        quantity: quantity ?? this.defaultIngredientAmount,
                        quantityIsFraction: wasFraction,
...
function parseMixedFraction(mixedFraction: string) {
    console.log('parsing mixed fraction:', mixedFraction);
    ...
    console.log('got', whole + fractional);
    return whole + fractional;
}

function parseNonMixedFraction(fraction: string) {
    console.log('parsing non-mixed fraction:', fraction);
    const [numerator, denominator] = fraction.split('/').map(Number);
    console.log('got', numerator / denominator);
